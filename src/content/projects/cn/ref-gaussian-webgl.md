...
部署浏览器端体验，使其在常规硬件上达到 **60+ FPS**，且支持光泽/反射表面等 PBR 属性。
...
- 在消费级笔记本（RTX 4060 Laptop）上实现 60+ FPS 的实时 WebGL Ref-Gaussian 渲染；
